...
function InfoRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-border last:border-0">
      ...
      <span className="text-sm font-medium text-foreground max-w-[55%] text-right truncate">{value || "—"}</span>
    </div>
  )
}
...
type BadgeTierMeta = Record<string, { label: string; color: string; bg: string; border: string }>
type BadgeCategoryMeta = Record<string, { label: string; color: string }>
...
export default function ProfileClient({
  ...
  const hireDateFormatted = hireDate
    ? new Date(hireDate).toLocaleDateString("en-US", { month: "short", year: "numeric" })
    : null
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative bg-card border-b border-border overflow-hidden">
          ...
          <div className="flex items-start gap-4 md:gap-6">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h1 className="text-xl md:text-2xl font-bold text-foreground tracking-tight">{fullName}</h1>
                  <p className="text-sm text-muted-foreground mt-0.5">{email}</p>
                </div>
                {username && (
                  <div className="flex-shrink-0 px-2.5 py-1 bg-secondary border border-border rounded-md">
                    <span className="text-[10px] font-mono font-medium text-muted-foreground">{username}</span>
                  </div>
                )}
              </div>
        ...
        <div className="grid grid-cols-4 border-t border-border">
          {[
            ...
            { icon: <Target className="h-3.5 w-3.5 text-amber-400" />, value: avgScore > 0 ? `${avgScore}%` : "—", label: "Avg Score", border: true },
            { icon: <BookOpen className="h-3.5 w-3.5 text-blue-400" />, value: `${completedModules}/${totalModules}`, label: "Modules", border: true },
            { icon: <Award className="h-3.5 w-3.5 text-purple-400" />, value: `${earnedBadgesCount}`, label: "Badges", border: true },
          ].map((stat, i) => (
      ...
      <div className="p-4 md:p-8 space-y-6">
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {completedModules} completed &middot; {inProgressModules} in progress &middot; {notStartedModules} not started
                  </p>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-border">
                <div className="text-center p-2.5 rounded-lg bg-primary/5 border border-primary/10">
                  ...
                  <p className="text-lg font-bold text-primary">{completedModules}</p>
                ...
                <div className="text-center p-2.5 rounded-lg bg-amber-400/5 border border-amber-400/10">
                  ...
                  <p className="text-lg font-bold text-amber-400">{inProgressModules}</p>
                ...
                <div className="text-center p-2.5 rounded-lg bg-secondary border border-border">
                  ...
                  <p className="text-lg font-bold text-foreground">{notStartedModules}</p>
        ...
        {/* Tabs: Modules / Quiz History / Achievements */}
        <div>
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border hover:border-primary/30 transition-all group">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {mod.completed && <span className="text-xs text-primary font-medium">Completed</span>}
          ...
          {activeTab === "quizzes" && (
            <div>
              <div className="flex items-center gap-3 p-4 rounded-xl bg-card border border-border mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-foreground">{quizResults.length} quizzes completed</p>
                  <p className="text-xs text-muted-foreground mt-0.5">Average score across all attempts</p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-primary">{avgScore > 0 ? `${avgScore}%` : "—"}</p>
                    ...
                    quizResults.map((quiz, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-foreground">{quiz.quiz_title}</p>
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              <span className="text-xs text-muted-foreground">
                                {new Date(quiz.completed_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                              </span>
                            </div>
                          </div>
                          <Badge
          ...
          {activeTab === "badges" && (
            <div>
              <div className="flex items-center gap-3 p-4 rounded-xl bg-card border border-border mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-foreground">{earnedBadgesCount} badges earned</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {allBadgeDefinitions.length - earnedBadgesCount} more to unlock
                  </p>
                </div>
                <Link href="/badges">
              ...
              {earnedBadgeIds.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center mb-4">
                  ...
                  <p className="text-xs text-muted-foreground/70 mt-1">Complete training modules and quizzes to earn your first badge</p>
                </div>
              )}
        ...
        <div>
          ...
          <Card className="border-border">
            <CardContent className="px-4 py-1">
              ...
              <InfoRow icon={<Briefcase className="h-4 w-4" />} label="Role" value={role} />
              <InfoRow icon={<MapPin className="h-4 w-4" />} label="Location" value={storeLabel} />
              <InfoRow icon={<Hash className="h-4 w-4" />} label="Username" value={username} />
              {hireDateFormatted && (
                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Hire Date" value={hireDateFormatted} />
              )}
            </CardContent>
          </Card>
        </div>
