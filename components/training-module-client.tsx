...
import type { mockModules } from "@/lib/data"
...
type Module = (typeof mockModules)[number]
...
function LessonContent({ content }: { content: string }) {
  ...
  while (i < lines.length) {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-secondary/80">
                  {headers.map((h, idx) => (
                    <th key={idx} className="px-3 py-2.5 text-left text-xs font-semibold text-foreground border-b border-border">{renderInline(h)}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((row, ridx) => (
                  <tr key={ridx} className="border-b border-border/50 last:border-0 hover:bg-secondary/30 transition-colors">
                    {row.map((cell, cidx) => (
                      <td key={cidx} className="px-3 py-2.5 text-xs text-muted-foreground">{renderInline(cell)}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
    ...
    if (line.startsWith("- ")) {
      const items: string[] = []
      while (i < lines.length && lines[i].startsWith("- ")) { items.push(lines[i].substring(2)); i++ }
      elements.push(
    ...
    elements.push(
      <p key={i} className="text-sm text-muted-foreground leading-relaxed mb-3">{renderInline(line)}</p>
    )
    i++
  }
...
function CompletionBanner({ quizId }: { quizId: string }) {
  return (
...
interface Props {
  ...
  onSaveProgress: (moduleId: string, completedLessonIds: string[], totalLessons: number) => Promise<void>
}

export default function TrainingModuleClient({ module, initialCompletedLessonIds, onSaveProgress }: Props) {
  const [currentLessonIndex, setCurrentLessonIndex] = useState(() => {
    const firstIncomplete = module.lessons.findIndex(
      (l) => !initialCompletedLessonIds.includes(l.id)
    )
  ...
  const handleComplete = async () => {
    ...
    setSaving(true)
    try {
      await onSaveProgress(module.id, Array.from(next), module.lessons.length)
    } finally {
      setSaving(false)
    }
        ...
        <aside className={cn(
          ...
          <div className="px-4 py-4 border-b border-border shrink-0">
            ...
            <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
              <span className="flex items-center gap-1"><Clock className="h-3 w-3" />{module.estimatedTime}</span>
              <span className="flex items-center gap-1"><BookOpen className="h-3 w-3" />{module.lessons.length} lessons</span>
            </div>
          </div>
                ...
                >
                  <div className="mt-0.5 shrink-0">
                    {isDone ? <CheckCircle2 className="h-4 w-4 text-primary" />
                      : isCurrent ? <PlayCircle className="h-4 w-4 text-primary" />
                      : <Circle className="h-4 w-4" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={cn("text-xs font-medium leading-snug", isCurrent && "text-primary")}>{lesson.title}</p>
          ...
          <div ref={contentRef} onScroll={handleScroll} className="flex-1 overflow-y-auto">
            <div className={cn(
              ...
              <div className="mt-10 pt-6 border-t border-border">
                {allDone ? (
                  <CompletionBanner quizId={module.quizId} />
                ) : (
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <Button
                      ...
                      disabled={saving}
                      className={cn(
                        isCurrentDone && currentLessonIndex < module.lessons.length - 1 && "bg-primary/80"
                      )}
                    >
                      {saving ? (
                        ...
                        currentLessonIndex < module.lessons.length - 1 ? (
                          ...
                          <>All Done <CheckCircle2 className="h-3.5 w-3.5 ml-1.5" /></>
                        )
                      ) : (
                        <>Mark Complete <CheckCircle2 className="h-3.5 w-3.5 ml-1.5" /></>
                      )}
                    </Button>
                  </div>
                )}
