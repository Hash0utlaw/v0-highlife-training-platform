...
type Module = (typeof mockModules)[number] & { progress: number; completed: boolean }
...
export default function TrainingListClient({ modules }: { modules: Module[] }) {
  const [search, setSearch] = useState("")

  const completedCount = modules.filter((m) => m.completed).length
  const inProgressCount = modules.filter((m) => !m.completed && m.progress > 0).length
  const overallProgress = modules.length > 0
    ? Math.round((completedCount / modules.length) * 100)
    : 0

  const filtered = useMemo(() => {
    if (search.length < 2) return modules
